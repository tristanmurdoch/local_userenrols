<?php
    ...
    $string['LBL_IMPORT_TITLE']         = 'Import Group Enrollment File';
    ...
    $string['shortnamefilter'] = 'Filter';
    $string['shortnamefilter_desc'] = 'Filter Explain';
    $string['userenrols_config_title'] = 'UserEnrols Custom';
    $string['userenrols:access'] = 'Upload groups via tool';
    $string['HELP_PAGE_IMPORT']         = 'Import Enrollments & Group Assignments';
    $string['HELP_PAGE_IMPORT_help']    = '
<p>
Use this group import plugin to import group enrollments from a delimited text
file into the course. New user accounts will not be created, so each of the
users listed in the input file must already have an account set up in the site
 and be currently enrolled within the course.<br />
</p>

<ul>
  <li>Each line of the import file represents a signle record</li>
  <li>Each record should at least contain one field with a userid value,</li>
  <li>Each record must contain an additional group name field, separated by a comma, semi-colon, or tab character.</li>
  ...
  <li>Note: If a user is NOT already enrolled in the course, no changes will be made to that user\'s enrollment (i.e. not enrolled into course).</li>
</ul>

<?php
    ...
    class local_userenrols_plugin
    {
        ...
        private static $course_id_field_options    = null;
         ...
         * @return array
         */
        public static function get_course_id_field_options()
        {
            if (self::$course_id_field_options == null) {
                self::$course_id_field_options = array(
                    '9456' => 'BUS340tja_18',
                    '9502' => 'BUS293tja_18'
                );
            }

            return self::$course_id_field_options;

        }

        private static $courses_all_options    = null;

        public static function get_courses_all_options() {
        global $DB;
        self::$courses_all_options = $DB->get_records('course',null, null, 'id,shortname');

        return self::$courses_all_options;
         ...
         * @uses $DB
         */
        public static function import_file($course, stdClass $enrol_instance, $ident_field, $role_id, $group_assign, $group_id, $group_create, stored_file $import_file)
        {
            global $DB;
            ...
            if ($group_assign) {
                ...
                if (false === ($existing_groups = groups_get_all_groups($course))) {
            ...
            $metacourse = false;
            $enrols_enabled = enrol_get_instances($course, true);
        ...
        public static function get_courses_filtered($settings) {

            global $CFG,$DB;

            $results = array();
            //$code  = 'BSC.*|MBS.*_18|BUS.*_19|MBS.*_19|POL.*_18';
            $allcourses = local_userenrols_plugin::get_courses_hidden();

            foreach($allcourses as $allcourse) {
                if (preg_match('/'.$settings->shortnamefilter.'/', $allcourse->shortname, $matches)){
                    $results[] = $allcourse;
                }
            }
        return($results);
        ...
        private static function get_courses_hidden($categoryid="all", $sort="c.sortorder ASC", $fields="c.*") {
        ...
        if ($categoryid !== "none" && is_numeric($categoryid)) {
            $categoryselect = "WHERE c.category != :catid";
            $params['catid'] = $categoryid;
        } else {
            $categoryselect = "";
